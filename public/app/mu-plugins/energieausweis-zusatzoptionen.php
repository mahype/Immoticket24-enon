...
function energieausweis_zusatzoptionen_get_default( $field = '' ) {
	switch ( $field ) {
		...
		case 'premium_bewertung_label':
			return '<img src="' . get_template_directory_uri() . '/assets/media/neu.png" style="height: 30px;" /> Premium-Bewertung Ihrer Immobilie - KOSTENLOSE TESTPHASE!';
...
function energieausweis_zusatzoptionen_send_to_experten_check_email_address( $emails ) {
	if ( ! in_array( 'angabencheck@energieausweis-online-erstellen.de', $emails, true ) ) {
		$emails[] = 'angabencheck@energieausweis-online-erstellen.de';
	}

	return $emails;
}

function energieausweis_zusatzoptionen_send_to_sendung_per_post_email_address( $emails ) {
	if ( ! in_array( 'postversand@energieausweis-online-erstellen.de', $emails, true ) ) {
		$emails[] = 'postversand@energieausweis-online-erstellen.de';
	}

	return $emails;
}

function energieausweis_zusatzoptionen_send_to_energieausweis_besprechung_email_address( $emails ) {
	if ( ! in_array( 'angabencheck@energieausweis-online-erstellen.de', $emails, true ) ) {
		$emails[] = 'angabencheck@energieausweis-online-erstellen.de';
	}

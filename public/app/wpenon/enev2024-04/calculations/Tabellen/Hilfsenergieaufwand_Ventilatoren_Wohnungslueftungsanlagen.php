...
class Hilfsenergieaufwand_Ventilatoren_Wohnungslueftungsanlagen
{
	...
	/**
	 * Stromart 'ac' oder 'dc_ec'
	 *
	 * @var int
	 */
	protected string $strom_art;
	 ...
	 * @param string $strom_art 'ac' oder 'dc_ec'.
	 *
	 * @return void
	 */
	public function __construct(float $flaeche, string $strom_art)
	{
		$this->flaeche    = $flaeche;
		$this->strom_art  = $strom_art;
	...
	protected function interpolierter_wert(): float
	{
		...
		foreach ($this->flaecheslug() as $flaeche_slug) {
			...
			$strom_art = $this->strom_art;
			$values[] = (float) $this->table_data[$flaeche_slug]->$strom_art;
		}

...
function wpenon_get_enev_pdf_data($context, $index = 0, $energieausweis = null, $data = array())
{
	if (isset($energieausweis) && $energieausweis->mode == 'v') {
		$calcCC = new Enev\Schema202403\Calculations\CalculationsCC($energieausweis);
	}
	...
	switch ($context) {
		...
		case 'ht_reference':
			...
			if (isset($calculations['ht_strich_ref'])) {
				return $calculations['ht_strich_ref'];
			}
		...
		case 'primaerenergie_reference':
			...
			if (isset($calculations['Qp_ref'])) {
				return $calculations['Qp_ref'];
			}
		...
		case 'modernisierungsempfehlungen':
			if($energieausweis->mode == 'v') {
				$modernisierungen = new Enev\Schema202403\Modernizations\VW_Modernizations($energieausweis);
			} else {
				$modernisierungen = new Enev\Schema202403\Modernizations\BW_Modernizations($energieausweis);
			}

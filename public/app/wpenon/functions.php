    ...
    <div class="modal-dialog" role="document" style="margin-top:140px;">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal"><?php _e( 'Angaben bearbeiten', 'wpenon' ); ?></button>
          <button id="wpit_transfer_certificate" type="button" class="wpit_transfer_certificate btn btn-primary"><?php _e( 'In Bedarfsausweis umwandeln', 'wpenon' ); ?></button>
        </div>
      </div>
    </div>
  </div>
...
function wpenon_immoticket24_get_energietraeger_name( $slug, $is_with_units = false ) {
  if ( $is_with_units ) {
    $slug = wpenon_get_table_results( 'energietraeger_umrechnungen', array( 'bezeichnung' => array( 'value' => $slug, 'compare' => '=' ) ), array( 'energietraeger' ), true );
  }
  ...
  return wpenon_get_table_results( $table_slug, array( 'bezeichnung' => array( 'value' => $slug, 'compare' => '=' ) ), array( 'name' ), true );
}

function wpenon_immoticket24_get_fenster_bauarten() {
  $_fenster = wpenon_get_table_results( 'uwerte', array( 'bezeichnung' => array( 'value' => 'fenster_%', 'compare' => 'LIKE' ) ), array( 'name' ), false, 'name', 'ASC' );
  ...
  <div style="max-width:600px;margin:0 auto;">
    ...
    <table class="table table-bordered" style="background-color:rgba(255,255,255,0.1)">
      ...
      <tbody>
        ...
        <tr>
          <th headers="fenster_tabelle_bauart" scope="row">2fach Wärmedämmglas<br>(2 Scheiben mit einer reflektierenden Beschichtung im Scheibenzwischenraum)</th>
          <td>ab 1995</td>
          <td>1,1</td>
        </tr>
        <tr>
          <th headers="fenster_tabelle_bauart" scope="row">3fach Wärmedämmglas<br>(3 Scheiben mit zwei reflektierenden Beschichtungen im Scheibenzwischenraum)</th>
          <td>ab 2005</td>
          <td>0,7</td>
        </tr>
      </tbody>
...
function wpenon_immoticket24_show_gebaeude_table() {
  ...
  <div style="max-width:600px;margin:0 auto 15px;font-size:120%;">
    ...
    <div style="padding:5px 10px;border:1px solid #333;border-top:0;">
      <span style="display:inline-block;min-width:170px;">einseitig angebaut</span> <span style="display:inline-block;width:30px;text-align:center;">=</span> Reiheneckhaus, Doppelhaushälfte
      <br>
      <span style="display:inline-block;min-width:170px;">zweiseitig angebaut</span> <span style="display:inline-block;width:30px;text-align:center;">=</span> Reihenhaus
    </div>
  </div>
  <?php
}

...
function wpenon_immoticket24_get_energietraeger_name_2021($slug, $is_with_units = false)
{
	...
	switch ($slug) {
		case 'fernwaermekwkfossil':
			return 'Nahversorgung';
		case 'fernwaermehzwfossil':
			return 'Fernheizwärme';
	}
...
function wpenon_immoticket24_send_needs_review_email($payment_id, $reason)
{
	...
	$slack_text = 'Neue Bestellung ' . $payment_title . ' - Überprüfung benötigt. Grund: ' . $reason . "\n";
	...
	sendSlackNotification($slack_webhook_url, $slack_text );
...
function sendSlackNotification($webhook_url, $text) {
    $data = ['text' => $text];
    $options = [
        'http' => [

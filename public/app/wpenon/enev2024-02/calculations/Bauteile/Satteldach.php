		...
		switch ( $this->grundriss->form() ) {
			...
			case 'b':
				if ( $this->grundriss->wand_laenge( 'a' ) > $this->grundriss->wand_laenge( 'b' ) ) {
					...
					$flaeche                 += 2 * ( $this->grundriss->wand_laenge( 'b' ) - 0.25 * $this->grundriss->wand_laenge( 'f' ) ) * $dach_s1 + 2 * ( $this->grundriss->wand_laenge( 'e' ) + 0.25 * $this->grundriss->wand_laenge( 'c' ) ) * $dach_s2;
					$volumen                 += 0.5 * $this->grundriss->wand_laenge( 'b' ) * $this->grundriss->wand_laenge( 'c' ) * $this->hoehe() + 0.5 * $this->grundriss->wand_laenge( 'e' ) * $this->grundriss->wand_laenge( 'f' ) * $this->hoehe() + ( 1.0 / 3.0 ) * ( 0.5 * $this->grundriss->wand_laenge( 'f' ) * $this->hoehe() ) * ( 0.5 * $this->grundriss->wand_laenge( 'c' ) );
			...
			case 'c':
				...
				$dach_s2 = sqrt( pow( $this->hoehe(), 2 ) + pow( 0.5 * $this->grundriss->wand_laenge( 'e' ), 2 ) );
				// array_push( $dachwinkel, atan( $this->hoehe() / ( 0.5 * $this->grundriss->wand_laenge( 'b' ) ) ), atan( $this->hoehe() / ( 0.5 * $this->grundriss->wand_laenge( 'e' ) ) ) );
				$flaeche                 += 2 * ( $this->grundriss->wand_laenge( 'a' ) - 0.25 * $this->grundriss->wand_laenge( 'e' ) ) * $dach_s1 + 2 * ( $this->grundriss->wand_laenge( 'd' ) + 0.25 * $this->grundriss->wand_laenge( 'b' ) ) * $dach_s2;
				$volumen                 += 0.5 * $this->grundriss->wand_laenge( 'a' ) * $this->grundriss->wand_laenge( 'b' ) * $this->hoehe() + 0.5 * $this->grundriss->wand_laenge( 'e' ) * $this->grundriss->wand_laenge( 'd' ) * $this->hoehe() + ( 1.0 / 3.0 ) * ( 0.5 * $this->grundriss->wand_laenge( 'e' ) * $this->hoehe() ) * ( 0.5 * $this->grundriss->wand_laenge( 'b' ) );
				$this->wand_flaechen['b'] = 0.5 * $this->grundriss->wand_laenge( 'b' ) * $this->hoehe();
				$this->wand_flaechen['e'] = 0.5 * $this->grundriss->wand_laenge( 'e' ) * $this->hoehe();

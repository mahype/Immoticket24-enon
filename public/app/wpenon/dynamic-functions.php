...
function wpenon_immoticket24_is_h_erzeuger_ww( $erzeuger ) {
  $ww = wpenon_get_table_results( 'ww_erzeugung', array( 'bezeichnung' => array( 'value' => $erzeuger, 'compare' => '=' ) ), array( 'name' ), true );
  if ( $ww ) {
    return true;
  }
  return false;
...
function wpenon_immoticket24_get_energietraeger_unit( $energietraeger ) {
  return wpenon_get_table_results( 'energietraeger_umrechnungen', array( 'bezeichnung' => array( 'value' => $energietraeger, 'compare' => '=' ) ), array( 'einheit' ), true );
}

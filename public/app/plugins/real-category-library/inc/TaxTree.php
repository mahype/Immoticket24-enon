...
class TaxTree implements \DevOwl\RealCategoryLibrary\overrides\interfce\IOverrideTaxTree {
    ...
    const SKIP_POST_TYPES = [
        'wp_block',
        'attachment',
        // [Plugin Comp] https://wordpress.org/plugins/insert-php/
        // The plugin does not `register_taxonomy` in general, it uses the condition `is_admin` so
        // it is never registered for REST API requests.
        'wbcr-snippets'
    ];
    /**
    ...
    public function __construct(
        ...
        foreach ($taxonomy_objects as $key => $value) {
            if (\boolval($value->hierarchical)) {
                 ...
                 * @hook RCL/Available
                 */
                $isTaxonomyEnabled = apply_filters('RCL/Available', \true, $this->getTypeNow(), $key);

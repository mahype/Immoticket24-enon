...
if (!\class_exists('DevOwl\\RealCategoryLibrary\\Vendor\\Puc_v4p11_Plugin_UpdateChecker', \false)) {
    ...
    class Puc_v4p11_Plugin_UpdateChecker extends \DevOwl\RealCategoryLibrary\Vendor\Puc_v4p11_UpdateChecker
    {
        ...
        public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = '')
        {
            ...
            if ($slugUsedBy) {
                $this->triggerError(\sprintf('Plugin slug "%s" is already in use by %s. Slugs must be unique.', \htmlentities($slug), \htmlentities($slugUsedBy)), \E_USER_ERROR);
            }
        ...
        {
            $scheduler = new \DevOwl\RealCategoryLibrary\Vendor\Puc_v4p11_Scheduler($this, $checkPeriod, array('load-plugins.php'));
        ...
        {
            $relevant = $action == 'plugin_information' && isset($args->slug) && ($args->slug == $this->slug || $args->slug == \dirname($this->pluginFile));
            if (!$relevant) {
                return $result;
            }
        ...
        {
            return \array_merge(parent::getNoUpdateItemFields(), array('id' => $this->pluginFile, 'slug' => $this->slug, 'plugin' => $this->pluginFile, 'icons' => array(), 'banners' => array(), 'banners_rtl' => array(), 'tested' => '', 'compatibility' => new \stdClass()));
        }
        /**

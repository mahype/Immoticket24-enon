...
class Assets {
    ...
    public function overrideLocalizeScript($context) {
        ...
        return [
            'currentUserFullName' => \DevOwl\RealCategoryLibrary\Vendor\DevOwl\RealProductManagerWpClient\Utils::getUserFullName(),
            'names' => $names
        ];
    }
    /**
    ...
    public function enqueue_scripts_and_styles($type, $hook_suffix = null) {
        $isPluginsPage = \function_exists('get_current_screen')
            ? \in_array(get_current_screen()->id, ['plugins', 'plugins-network'], \true)
            : \false;
        if (!$isPluginsPage) {
            return;
        }

...
class OptimizePress_Elements_VideoBackground
{
    ...
    public function addRowOptions()
    {
        ...
        <div class="op-row-video-background-form">
            ...
            <p class="op-micro-copy"><?php _e('Please note that the video will always be muted in Safari, because it blocks autoplay for videos with sound', 'optimizepress-plus-pack');?></p>
    ...
    public function renderVideoMarkup($options)
    {
        ...
        if (isset($data['enqueue_youtube_api']) && $data['enqueue_youtube_api'] == 1) {
            if (OP_SCRIPT_DEBUG === '') {
                ...
                wp_enqueue_script(OP_SN . '-on-youtube-iframe-api-ready', OP_LIB_URL.'modules/blog/video/on_youtube_iframe_api_ready'.OP_SCRIPT_DEBUG.'.js', array(OP_SN . 'plus-pack-js-front-all'), OP_VERSION, true);
            }
        }

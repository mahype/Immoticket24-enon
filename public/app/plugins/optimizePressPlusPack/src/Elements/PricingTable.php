...
class OptimizePress_Elements_PricingTable
{
    ...
    public function elementOpppStyles($styles, $id)
    {
        if ('op_assets_core_' . $this->parentTag . '_style' === $id) {
            $styles = array_merge($styles, array(4, 5, 6, 7, 8, 9 ,10, 11));
        }
    ...
    {
        // Element id
        $this->id = 'pt_' . md5(serialize($atts) . $content);
    ...
    public function childShortcode($atts, $content)
    {
        ...
        // Element id
        $tableId = 'el_' . md5(serialize($atts) . $content);
    ...
    public function addToLangList($strings)
    {
        ...
        $strings['pricing_table_button_color'] = __('Button Color', 'optimizepress-plus-pack');
        $strings['pricing_table_price_color'] = __('Price Color', 'optimizepress-plus-pack');
        $strings['pricing_table_color_picker'] = __('Pricing Table Color Picker', 'optimizepress-plus-pack');

...
class OptimizePress_Elements_AnimatedElements
{
        ...
        if (false === $fields = get_transient($cacheKey)) {
            $fields = op_sl_parse('animated_elements', array(
                ...
                'labels'    => array(
                    ...
                    'animate_mobile' => __('Do not animate on mobile devices.', 'optimizepress-plus-pack'),
                ),
            ));
    ...
    public function parseAdvancedOptions($element)
    {
        ...
        if ( ! empty($element['element_data_style'])) {
            ...
            // Check if animation is required for mobile
            if(wp_is_mobile()){
                if(isset($dataStyle->extras->dontAnimateOnMobile)){
                    if($dataStyle->extras->dontAnimateOnMobile === 1){
                        return $element;
                    }
                }
            }

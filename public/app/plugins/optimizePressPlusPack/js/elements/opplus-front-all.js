    ...
    function singleLetters($words) {
        $words.each(function(){
            ...
            for (i in letters) {
                ...
                // We want effects (type & type fast) to behave as
                // regular text (break into multiple lines),
                // while other effects need to have nowrap
                // set, because of trasform animations
                if (!headline.hasClass('type') && !headline.hasClass('type_fast')) {
...
;(function ($) {
    ...
    function setPricingTableStyleHeights() {
        ...
        $('.pricing-table-style4, .pricing-table-style5, .pricing-table-style6, .pricing-table-style7, .pricing-table-style8, .pricing-table-style9', '.pricing-table-style10', '.pricing-table-style11').each(function () {
    ...
    if ($('.op-row-video-background-wrap').length > 0 || $('.op-row-video-background-template').length > 0) {
        ...
        function resizeVideo() {
            ...
            $rowVideoWraps.each(function(i) {
                ...
                // false is intentionally string, since
                // it's read from the attribute
                if (!$.browser.safari && $video.attr('data-muted') === 'false') {
...
function opUnmuteYoutubeVideo(event) {
    // Safari stops/blocks autoplaying videos,
    // so we're forcing background
    // videos to stay muted
    if (opjq.browser && opjq.browser.safari === true) {
        return false;
    }
    event.target.unMute();
};

function initVideoBackgroundYoutubeElements() {
    var $ = opjq;
    var videoTimeout;

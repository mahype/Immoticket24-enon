 ...
 */
class ServiceGroupCommand extends MainCommand
{
    /**
     ...
     * : Whether or not the service group will be pre selected in the cookie box.
     *
     * [--status=<status>]
     * : Whether or not the service group will be activated.
     *
     * [--undeletable=<undeletable>]
     * : Whether or not the service group will be undeletable.
     *
     * [--porcelain]
    ...
    public function create(array $args, array $assocArgs): void
    {
        ...
        $this->serviceGroupRepository->insert($serviceGroup);

        if ($porcelain) {
            WP_CLI::line($serviceGroup->id);
        } else {
            WP_CLI::success('Created service group ' . $serviceGroup->id);
        }
    }
    ...
    {
        $serviceGroup = $this->serviceGroupRepository->findById((int) ($args[0]), true);
    ...
    public function list(array $args, array $assocArgs): void
    {
        ...
        $iterator = WP_CLI\Utils\iterator_map(
            ...
            function ($serviceGroup) {
                return $this->mapToCliTable($serviceGroup);
            },
        );
    ...
    public function update(array $args, array $assocArgs): void
    {
        ...
        if ($position !== null) {
            $serviceGroup->position = $position;
        }
        ...
        if ($preSelected !== null) {
            $serviceGroup->preSelected = $preSelected;
        }
        ...
        if ($undeletable !== null) {
            $serviceGroup->undeletable = $undeletable;
        }
    ...
    private function mapToCliTable(ServiceGroupModel $serviceGroupModel): array
    {
        return [
            ...
            'pre-selected' => $serviceGroupModel->preSelected,
            'position' => $serviceGroupModel->position,
            'undeletable' => $serviceGroupModel->undeletable,
            'status' => $serviceGroupModel->status,
            'services' => array_map(
                function ($service) {
                    return $service->id;
                },

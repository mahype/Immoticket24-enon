...
class LicenseCommand extends AbstractCommand
{
    ...
    public function info(array $args, array $assocArgs): void
    {
        ...
        $licenseCloudScans = Searcher::findObject($licenseData->licenseMeta->list, 'key', 'scans')->value ?? '';
    ...
    public function register(array $args, array $assocArgs): void
    {
        ...
        $globalLocalization = GlobalLocalizationStrings::get();

        try {
            $this->license->register($licenseKey);
        } catch (GenericException $e) {
            WP_CLI::error(
                $localization['alert'][$e->getMessage()]
                ?? Formatter::interpolate($globalLocalization['alert']['unknown'], ['message' => $e->getMessage()]),
            );
        }

...
final class ContentBlockerController implements ControllerInterface
{
    ...
    public function save(int $id, array $postData): string
    {
        ...
        if ($contentBlocker !== null && isset($postData['languages'])) {
            $additionalLanguages = array_keys(
                array_filter(
                    $postData['languages'],
            ...
            $this->contentBlockerService->getOrCreateContentBlockerPerLanguage($contentBlocker, $additionalLanguages);
        }
    ...
    public function viewEdit(int $id, array $postData, array $getData): string
    {
        ...
        $templateData['data']['providerId'] = $contentBlocker->provider->id ?? $unknownProvider[0]->id ?? null;

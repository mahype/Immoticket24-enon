...
class Factory
{
     ...
     */
    public static function new(RepositoryInterface $repository, object $args): AbstractModel
    {
        ...
        try {
            ...
            foreach ($args as $fieldName => $argsValue) {
                ...
                if (class_exists($propertyTypeName) && (self::isDto($propertyTypeName) || self::isDtoList($propertyTypeName))) {
                    $instance->{$propertyName} = $propertyTypeName::fromJson(json_decode($argsValue ?? '{}')); // Todo remove {} ?

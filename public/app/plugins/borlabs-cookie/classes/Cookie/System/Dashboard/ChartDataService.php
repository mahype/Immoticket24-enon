...
class ChartDataService
{
    ...
    {
        $chartDataValues = [];
        $dayEntries = $this->consentStatisticService->getLastDays(7);
    ...
    {
        $chartDataValues = [];
        $dayEntries = $this->consentStatisticService->getToday();

        foreach ($dayEntries as $dayEntry) {
            $hourKey = str_pad((string) $dayEntry->hour, 2, '0', STR_PAD_LEFT) . ':00';

            if (!isset($chartDataValues[$dayEntry->serviceGroupKey][$hourKey])) {
                $chartDataValues[$dayEntry->serviceGroupKey][$hourKey] = 0;
            }

            $chartDataValues[$dayEntry->serviceGroupKey][$hourKey] += $dayEntry->count;
        }

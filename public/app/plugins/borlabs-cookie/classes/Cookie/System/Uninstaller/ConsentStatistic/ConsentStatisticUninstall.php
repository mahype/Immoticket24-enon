...
final class ConsentStatisticUninstall
{
    ...
    public function uninstall(string $prefix = ''): bool
    {
        ...
        return
            ...
            && $this->wpdb->query('DROP TABLE IF EXISTS ' . $prefix . ConsentStatisticByDayGroupedByServiceGroupRepository::TABLE)
            && $this->wpdb->query('DROP TABLE IF EXISTS ' . $prefix . ConsentStatisticByHourRepository::TABLE)
            && $this->wpdb->query('DROP TABLE IF EXISTS ' . $prefix . ConsentStatisticByHourGroupedByServiceGroupRepository::TABLE);
    }
}

...
final class Language
{
    ...
    public function getCurrentLanguageCode(): string
    {
        ...
        if (!is_null($this->multilanguageContext)) {
            ...
            $this->log->debugExtended('Current language code: ' . $languageCode, [
                'requestUri' => $_SERVER['REQUEST_URI'],
            ]);
        }

        $finalLanguageCode = $this->determineLanguageCodeLength($languageCode);
        $this->log->debugExtended('Current language code after determining the code length: ' . $finalLanguageCode);

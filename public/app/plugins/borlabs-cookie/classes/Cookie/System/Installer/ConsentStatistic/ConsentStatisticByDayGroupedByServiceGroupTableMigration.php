...
final class ConsentStatisticByDayGroupedByServiceGroupTableMigration
{
    ...
    public function run(string $prefix = ''): AuditDto
    {
        ...
        // Rename table
        if (Database::tableExists($prefix . 'borlabs_cookie_consent_statistic_by_day_grouped') === true && Database::tableExists($prefix . self::TABLE) === false) {
            $renameStatus = $this->wpdb->query('
                RENAME TABLE
                    `' . $prefix . 'borlabs_cookie_consent_statistic_by_day_grouped`
                TO
                    `' . $prefix . self::TABLE . '`
            ');
            ...
            if ($renameStatus === false) {
                $this->log->error(
                    'Failed to rename table of ConsentStatisticByDayGroupedByServiceGroupRepository',
                    [
            ...
            $this->log->info(
                'Renamed table of ConsentStatisticByDayGroupedByServiceGroupRepository successfully',
            );
        }

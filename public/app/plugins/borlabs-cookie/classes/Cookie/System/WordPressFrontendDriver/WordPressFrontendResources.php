...
class WordPressFrontendResources
{
    ...
    public const BORLABS_COOKIE_MODULES = [
        ...
        'borlabs-cookie-prioritize-js-module',
    ];
    ...
    {
        if (in_array($attributes['id'], self::BORLABS_COOKIE_MODULES, true)) {
    ...
    public function registerJavaScriptModules(): void
    {
        ...
        $this->resourceEnqueuer->enqueueScriptModule('core', 'assets/' . $manifest[$borlabsCookieJSFile]['file'], $dependencies, null, true);
    }
    ...
    {
        $disableOnRestRequest = defined('REST_REQUEST') && $this->wpFunction->applyFilter('borlabsCookie/frontendResources/disabledOnRestRequest', true);
        $disableLoading = $this->wpFunction->applyFilter('borlabsCookie/frontendResources/disableCssLoading', false);

        return !$disableOnRestRequest && !$disableLoading;
    }

    private function shouldEnqueueJavaScriptResources(): bool
    {
        $disableOnRestRequest = defined('REST_REQUEST') && $this->wpFunction->applyFilter('borlabsCookie/frontendResources/disabledOnRestRequest', true);
        $disableLoading = $this->wpFunction->applyFilter('borlabsCookie/frontendResources/disableJavaScriptLoading', false);

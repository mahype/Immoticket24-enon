<?php
/**
 * Additional fee for pva_consultation.
 *
 * @category Class
 * @package  Enon\Tasks
 * @author   Sven Wagener
 ...
 * @since 1.0.0
 */
class Add_Costum_Fee_PVA_Consultation implements Filters, Task {
	/**
	...
	public function filter_fees( $fees ) {
		if ( ! $this->reason_allowed() && edd_is_checkout() ) {
			return $fees;
		}
	...
	public function filter_options( $options ) {
		$options['pva_consultation'] = array(
			'title'  => 'Kostenlose Erstberatung für Photovoltaik-Anlagen-Förderung',
			'fields' => array(
				'pva_consultation_label'       => array(
					'title'    => 'Name',
					'type'     => 'text',
					'default'  => $this->get_default_label(),
					'required' => true,
				),
				'pva_consultation_description' => array(
					'title'    => 'Beschreibung',
					'type'     => 'wysiwyg',
				...
				'pva_consultation_price'       => array(
					'title'   => 'Preis',
					'type'    => 'number',
					'default' => 0,
					'step'    => 0.01,
				),
				'pva_consultation_order'       => array(
					'title'       => 'Reihenfolge',
	...
	public function get_default_label() {
		return 'Kostenlose Erstberatung für Photovoltaik-Anlagen-Förderung';
	}

	public function get_default_description() {
		return 'Hier können Sie sich eine kostenlose Erstberatung zur Beantragung der Photovoltaik-Anlagen-Förderung sichern.';
	}
	...
	public function get_label() {
		...
		if ( isset( $settings['pva_consultation_label'] ) ) {
			return $settings['pva_consultation_label'];
		} else {
	...
	public function get_description() {
		...
		if ( isset( $settings['pva_consultation_description'] ) ) {
			return $settings['pva_consultation_description'];
		} else {
	...
	public function get_price() {
		...
		if ( isset( $settings['pva_consultation_price'] ) ) {
			return $settings['pva_consultation_price'];
		} else {
	...
	public function get_order() {
		...
		if ( isset( $settings['pva_consultation_order'] ) ) {
			return $settings['pva_consultation_order'];
		} else {
	...
	public function filter_emails( array $emails, $payment_fees, $energieausweis ) : array {
		...
		$email = 'photovoltaik@immoticket24.de';
	...
	private function has_fee( $payment_fees ) {
		foreach ( $payment_fees AS $payment_fee ) {
			if ( $payment_fee['id'] === 'pva_consultation' ) {
				return true;
			}
		}

...
class Task_CPT_Reseller implements Task, Actions, Filters {
	...
	public function set_custom_column_values( $column, $post_id ) {
		...
		switch ( $column ) {
			case 'company_name':
				echo sprintf( '<a href="%s">%s</a>', get_edit_post_link( $post_id ), $reseller_data->get_company_name() );
				break;
			case 'contact_name':
				echo $reseller_data->get_contact_name();
				break;
			case 'contact_email':
				echo $reseller_data->get_contact_email();
				break;
			case 'iframe_url':

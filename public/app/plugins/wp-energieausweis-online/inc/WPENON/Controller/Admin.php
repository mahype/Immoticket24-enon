...
class Admin
{
      ...
      switch ( $column_name ) {
        ...
        case 'email':
          ...
          if ( strpos( $value, $user_info['email'] ) === 0 ) {
            $value = str_replace( $user_info['email'], '<a href="' . esc_url( add_query_arg( array( 'user' => urlencode( $user_info['email'] ), 'paged' => false ) ) ) . '">' . esc_html( $user_info['first_name'] . ' ' . $user_info['last_name'] ) . '</a>', $value );
          }
          break;
        case 'status':
  ...
  public function _getPaymentsRowActions( $row_actions, $payment ) {
    $row_actions['wpenon_receipt_view'] = '<a href="' . add_query_arg( array( 'edd-action' => 'wpenon_receipt_view', 'purchase_id' => $payment->ID ), admin_url( 'edit.php?post_type=download&page=edd-payment-history' ) ) . '">' . __( 'Rechnung ansehen', 'wpenon' ) . '</a>';
  ...
  public function _showFrontendActionMessages() {
    if ( isset( $_GET['frontend_action'] ) && isset( $_GET['frontend_action_id'] ) && isset( $_GET['frontend_action_status'] ) ) {
      ...
      $_SERVER['REQUEST_URI'] = remove_query_arg( array( 'frontend_action', 'frontend_action_id', 'frontend_action_status' ), $_SERVER['REQUEST_URI'] );
    }
  }

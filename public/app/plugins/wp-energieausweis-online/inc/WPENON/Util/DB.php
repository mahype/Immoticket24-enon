  ...
  public static function getResults( $table_slug, $rows = array(), $cols = array(), $single = false, $results_mode = OBJECT_K, $orderby = null, $order = 'ASC' ) {
    ...
    foreach ( $rows as $col_name => $col_data ) {
      ...
      if ( is_array( $value ) ) {
        $compare = "IN (";
        $compare_end = ")";
      } elseif ( ! in_array( $compare, array( "=", ">", "<", ">=", "<=", "LIKE" ) ) || ( $compare == "LIKE" && $type != "'%s'" ) ) {
        $compare = "=";
      }
  ...
  public static function getTableColumnNames( $table_slug ) {
    ...
    foreach ( $columns as $column ) {
      if ( isset( $schema['fields'][ $column->Field ] ) ) {
        $names[] = html_entity_decode( preg_replace_callback( '/([A-Za-z]+)<sup>([0-9]+)<\/sup>/', array( __CLASS__, '_formatColumnName' ), str_replace( array( '<sub>', '</sub>' ), '', $schema['fields'][ $column->Field ]['title'] ) ) );
      } else {
        $names[] = $column->Field;
      }
    }

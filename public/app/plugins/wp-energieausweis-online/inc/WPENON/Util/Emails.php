...
class Emails {
  ...
  public function _addEmailTags() {
    edd_add_email_tag( 'bank_account_info', __( 'Ihre Bankdaten aufgelistet', 'wpenon' ), 'text/html' == EDD()->emails->get_content_type() ? array( $this, '_emailTagBankAccountInfo' ) : array( $this, '_emailTagBankAccountInfoPlain' ) );
    edd_add_email_tag( 'pdf_link', __( 'Fügt einen Link hinzu, damit Benutzer direkt auf die PDF-Rechnung zugreifen können', 'wpenon' ), 'text/html' == EDD()->emails->get_content_type() ? array( $this, '_emailTagPDFLink' ) : array( $this, '_emailTagPDFLinkPlain' ) );
    edd_add_email_tag( 'pdf_link_hide_common', __( 'Fügt einen Link zur PDF-Rechnung hinzu, in angepasster Variante ohne Header und Footer', 'wpenon' ), 'text/html' == EDD()->emails->get_content_type() ? array( $this, '_emailTagPDFHideCommonLink' ) : array( $this, '_emailTagPDFHideCommonLinkPlain' ) );
    edd_add_email_tag( 'inline_receipt', __( 'Fügt eine Auflistung der Bestellung inklusive Preisen direkt in die Email ein', 'wpenon' ),  'text/html' == EDD()->emails->get_content_type() ? array( $this, '_emailTagInlineReceipt' ) : array( $this, '_emailTagInlineReceiptPlain' ) );
    edd_add_email_tag( 'manual_notice_seller', __( 'Zeigt eine Mitteilung (für den Verkäufer) an, dass die Bestellung mit einer manuellen Zahlungsweise getätigt wurde (falls zutreffend)' ), array( $this, '_emailTagManualNoticeSeller' ) );
    edd_add_email_tag( 'customer_contact_data', __( 'Zeigt die Kontaktdaten zum Kunden an.', 'wpenon' ), 'text/html' == EDD()->emails->get_content_type() ? array( $this, '_emailTagCustomerContactData' ) : array( $this, '_emailTagCustomerContactDataPlain' ) );
    if ( WPENON_POSTAL ) {
      edd_add_email_tag( 'postal_notice_customer', __( 'Zeigt eine Mitteilung (für den Kunden) an, dass als Sendung per Post bestellte Energieausweise in Kürze versendet werden (falls zutreffend)', 'wpenon' ), array( $this, '_emailTagPostalNoticeCustomer' ) );
      edd_add_email_tag( 'postal_notice_seller', __( 'Zeigt eine Mitteilung (für den Verkäufer) an, dass bestimmte Energieausweise zusätzlich als Sendung per Post bestellt worden sind (falls zutreffend)', 'wpenon' ), array( $this, '_emailTagPostalNoticeSeller' ) );
    }
  }
        ...
        <tbody>
          ...
          <tr>
            ...
            <?php if ( edd_item_quantities_enabled() ) : ?>
              <td></td>
            <?php endif; ?>
            <td class="edd_subtotal_label" style="text-align:right;"><strong><?php _e( 'Subtotal', 'easy-digital-downloads' ); ?></strong></td>
          ...
          <?php if ( edd_use_taxes() ) : ?>
            <tr>
              ...
              <?php if ( edd_item_quantities_enabled() ) : ?>
                <td></td>
              <?php endif; ?>
              <td class="edd_tax_label" style="text-align:right;"><strong><?php _e( 'Tax', 'easy-digital-downloads' ); ?></strong></td>
          ...
          <tr>
            ...
            <?php if ( edd_item_quantities_enabled() ) : ?>
              <td></td>
            <?php endif; ?>
            <td class="edd_total_label" style="text-align:right;"><strong><?php _e( 'Total Price', 'easy-digital-downloads' ); ?></strong></td>
  ...
  public function _emailTagManualNoticeSeller( $payment_id ) {
    if ( ! edd_is_payment_complete( $payment_id ) ) {
      ...
      if ( in_array( 'manual_handling', $supports ) ) {
        $payment_overview_url = add_query_arg( array( 'post_type' => 'download', 'page' => 'edd-payment-history' ), admin_url( 'edit.php' ) );
  ...
  public function _adjustEmailFooterText( $text ) {
    ...
    $text = sprintf( __( 'Diese Email wurde automatisch von %s versendet.', 'wpenon' ), '<a href="' . esc_url( home_url() ) . '">' . str_replace( array( 'http://', 'https://' ), '', esc_url( home_url() ) ) . '</a>' );

			...
			if ($this->schema->isField($field)) {
				...
				if ((string) $value !== (string) $old_value) {
					...
					if (is_admin()) {
						...
						$note = sprintf("%s (%s): '%s' -> '%s'", $field_params['label'], $field, $old_value, $value, wp_get_current_user()->user_login);
						if (!empty($payment_id)) {
	...
	public function getCalcCC()
	{
		...
		switch ($this->schema_name) {
			...
			case 'enev2022-01':
				...
				break;
			case 'enev2023-01':
				$calculations = new \Enev\Schema202301\Calculations\CalculationsCC($this);
				break;
			case 'enev2023-02':
				$calculations = new \Enev\Schema202302\Calculations\CalculationsCC($this);
				break;
			case 'enev2024-01':
				$calculations = new \Enev\Schema202401\Calculations\CalculationsCC($this);
				break;
		}
		return $calculations;
	}

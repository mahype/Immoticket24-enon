...
class Table {
  ...
  public function addToMenu( $topmenu = null ) {
    ...
    if ( is_array( $topmenu ) && isset( $topmenu['slug'] ) ) {
      $hook = add_submenu_page( $topmenu['slug'], $this->schema['title'], $this->schema['title'], WPENON_TABLE_CAP, $this->slug, array( $this, 'display' ) );
      ...
      $hook = add_menu_page( $this->schema['title'], __( 'Tabellen', 'wpenon' ), WPENON_TABLE_CAP, $this->slug, array( $this, 'display' ), 'dashicons-feedback', WPENON_MENU_POSITION + 1 );
      $topmenu = array(
        'slug'    => $this->slug,
